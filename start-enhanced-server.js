...
console.log(`${colors.cyan}Starting X-Seller-8 Inventory Management System...${colors.reset}`);
...
const requiredModules = ['express', 'http-proxy'];
...
for (const module of requiredModules) {
  try {
    ...
    console.error(`${colors.yellow}Required module "${module}" is not installed.${colors.reset}`);
    allModulesInstalled = false;
  }
}

if (!allModulesInstalled) {
  console.error(`${colors.yellow}Please install required modules with: npm install ${requiredModules.join(' ')}${colors.reset}`);
  process.exit(1);
}

// Start backend server (using our enhanced version)
...
const server = app.listen(3000, () => {
  console.log(`
${colors.cyan}X-Seller-8 is now running!${colors.reset}
