...
const SalesManager = require('./modules/sales_manager');
...
// Store sales managers cache
const salesManagers = new Map();

// Get or create SalesManager for a store
const getSalesManager = async (storePaths) => {
  const { root } = storePaths;
  
  if (!salesManagers.has(root)) {
    const salesManager = new SalesManager(storePaths);
    await salesManager.initializeSalesSystem();
    salesManagers.set(root, salesManager);
  }
  
  return salesManagers.get(root);
};
...
const requireActiveStore = async (req, res, next) => {
  try {
    ...
    // Add sales manager to request
    req.salesManager = await getSalesManager(req.storePaths);
...
app.delete('/api/stores/:storeId', async (req, res) => {
  try {
    ...
    // Clean up sales manager cache if present
    const storePaths = await storeManager.getStorePaths(storeId).catch(() => null);
    if (storePaths && salesManagers.has(storePaths.root)) {
      salesManagers.delete(storePaths.root);
    }
...
app.post('/api/sales/:month', requireActiveStore, async (req, res) => {
  try {
    ...
    if (!salesData || !Array.isArray(salesData) || salesData.length === 0) {
      return res.status(400).json({ success: false, error: 'Sales data must be a non-empty array' });
    }
    
    // Add sales data using the sales manager
    const result = await req.salesManager.addSalesData(month, salesData);
    
    res.status(201).json({
      success: true,
      message: `Added sales data for ${month}`,
      storeId: req.activeStore.id,
      storeName: req.activeStore.name,
      month,
    ...
    logger.error(`Error adding sales data: ${error.message}`);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to add sales data'
    });
  }
});

// Get sales data for a specific month
app.get('/api/sales/:month', requireActiveStore, async (req, res) => {
  try {
    const { month } = req.params;
    ...
    const salesData = await req.salesManager.getSalesDataForMonth(month);
    ...
    res.json({
      ...
      salesEntries: salesData.salesEntries,
      salesData: salesData.salesData
    });
  } catch (error) {
    logger.error(`Error getting sales data: ${error.message}`);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to get sales data'
    });
  }
});

// Generate monthly profit report
app.post('/api/reports/:month', requireActiveStore, async (req, res) => {
  try {
    const { month } = req.params;
    ...
    const result = await req.salesManager.generateMonthlyReport(month);
    
    res.status(201).json({
      success: true,
      message: `Generated profit report for ${month}`,
      storeId: req.activeStore.id,
      storeName: req.activeStore.name,
      month,
    ...
    logger.error(`Error generating profit report: ${error.message}`);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to generate profit report'
    });
  }
});

// Get list of available profit reports
app.get('/api/reports', requireActiveStore, async (req, res) => {
  try {
    // Get list of reports
    const reports = await req.salesManager.getReports();
...
// Get specific profit report
app.get('/api/reports/:month', requireActiveStore, async (req, res) => {
  try {
    const { month } = req.params;
    ...
    const reportPath = path.join(req.storePaths.root, 'reports', `${month}_profit_report.json`);
    
    if (!await fs.pathExists(reportPath)) {
      return res.status(404).json({
        success: false,
        error: `No profit report found for ${month}`
      });
    }
    ...
    logger.error(`Error getting profit report: ${error.message}`);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to get profit report'
    });
  }
});

// Get list of months with sales data
app.get('/api/sales-months', requireActiveStore, async (req, res) => {
  try {
    // Get list of months with sales data
    const months = await req.salesManager.getMonthsWithSalesData();
    ...
    logger.error(`Error getting months with sales data: ${error.message}`);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to get months with sales data'
    });
  }
});

// Delete a sales entry
app.delete('/api/sales/:month/:filename', requireActiveStore, async (req, res) => {
  try {
    const { month, filename } = req.params;
    
    if (!month || !filename) {
      return res.status(400).json({ success: false, error: 'Month and filename are required' });
    }
    
    // Delete sales entry
    await req.salesManager.deleteSalesEntry(month, filename);
    
    res.json({
      success: true,
      message: `Deleted sales entry: ${month}/${filename}`,
      storeId: req.activeStore.id,
      storeName: req.activeStore.name
    });
  } catch (error) {
    logger.error(`Error deleting sales entry: ${error.message}`);
    res.status(error.message.includes('not found') ? 404 : 500).json({
      success: false,
...
app.post('/api/chat', requireActiveStore, async (req, res) => {
  ...
  try {
    ...
    const contextMessage = `The following question is about the inventory and sales for the store: ${req.activeStore.name} (ID: ${req.activeStore.id}). Please provide information relevant to this specific store's inventory, sales, costs, and profitability.`;
...
function startServer(port) {
  const server = app.listen(port)
    ...
    .on('listening', () => {
      ...
      logger.info(`Server with multi-store and sales capability is running at http://localhost:${address.port}`);
    });
}
